...
class DatasetDCA(Dataset):
    ...
    def __init__(
        self,
        path_data: str | Path,
        params_file_biased : str | Path | None = None,
        params_file_unbiased : str | Path | None = None,
        ...
        for record in SeqIO.parse(self.path_data, 'fasta'):
            ...
            encoded_seq = encode_sequence(record.seq)
            
            if encoded_seq:
                self.mat.append(encoded_seq)
            else:
                print(f"Invalid character in {record.description}. Sequence has been removed.")
        ...
        self.params = {
            ...
            "gaps_lr" : torch.tensor([0.005], device=device, dtype=self.dtype),
        }
